...
try:
    ...
    # From debug, BLOCK 304 was "BAB IVHASIL PENELITIAN DAN PEMBAHASAN"
    start_idx = -1
    ...
    print("Found BAB IV body starting at block:", start_idx)
    
    # Let's find where 4.4.2 "Pengujian Parameter Optimasi Stok" starts
    fifo_test_idx = -1
    for idx, (btype, content) in enumerate(blocks):
    ...
    print("Found 4.4.2 starting at block:", fifo_test_idx)
    ...
    for idx in range(start_idx + 1, fifo_test_idx):
        ...
        if btype == 'p':
            ...
            main_headings = ['Hasil Perancangan', 'Hasil Penelitian', 'Hasil Pengujian', 'Pembahasan']
            sub_headings = [
            ...
            if is_heading:
                if h_level == 4:
                    num = ""
                    if heading_text == 'Hasil Perancangan': num = "4.2. "
                    elif heading_text == 'Hasil Penelitian': num = "4.3. "
                    elif heading_text == 'Hasil Pengujian': num = "4.4. "
                    html_lines.append(f"<h4>{num}{heading_text}</h4>")
                    
                    # Inject transition if exists
                    if heading_text in transitions:
                        html_lines.append(f'<p style="text-indent: 0.5in;">{transitions[heading_text]}</p>')
                else:
...
<body>
    ...
    with open(r'C:\Users\Ryan\Documents\TA\diagrams\Revisi_Lengkap_BAB_IV.html', 'w', encoding='utf-8') as f:
        f.write(html_output)
    print("Unified BAB IV successfully created with true tables.")
