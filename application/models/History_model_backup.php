    ...
    public function addTransaction(array $data): array
    {
        ...
        if ($typeNorm === 'Masuk') {
            ...
            try {
                ...
                // Update as_electric.stock from authoritative source: as_history.qty_sisa if available
                if ($this->db->table_exists($this->electricTable) && $this->db->field_exists('stock', $this->electricTable)) {
                    if ($this->db->field_exists('qty_sisa', $this->table)) {
                        $row = $this->db->select('SUM(qty_sisa) as total')->where('electric_id', $data['electric_id'])->get($this->table)->row_array();
        ...
        if ($this->db->field_exists('qty_sisa', $this->table)) {
            ...
            try {
                ...
                if ($this->db->table_exists($this->electricTable) && $this->db->field_exists('stock', $this->electricTable)) {
                    $row = $this->db->select('SUM(qty_sisa) as total')->where('electric_id', $electricId)->get($this->table)->row_array();
                    $newStock = isset($row['total']) ? (int) $row['total'] : 0;
