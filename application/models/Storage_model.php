...
{
    /**
     * Menyimpan apakah tabel `as_storage` ada di database.
     * Dideklarasikan untuk menghindari peringatan dynamic property di PHP 8.2+
     * @var bool
     */
    protected bool $has_storage_table = false;

    public function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->has_storage_table = $this->db->table_exists('as_storage');
        // Load history model for automatic transaction logging (if available)
        if (file_exists(APPPATH . 'models/History_model.php')) {
            $this->load->model('History_model');
        }
        // Load ID generator for manual ID enforcement when needed
        if (file_exists(APPPATH . 'models/Id_generator_model.php')) {
    ...
    {
        if (empty($this->has_storage_table)) return [];
    ...
    {
        if (empty($this->has_storage_table)) return false;
    ...
    public function add_storage($data)
    {
        ...
        // If table has numeric `id` column, generate manual id first
        if ($this->db->field_exists('id', 'as_storage') && isset($this->Id_generator_model)) {
            $storage_data_db['id'] = (int)$this->Id_generator_model->generate_manual_id('as_storage', 'id');
        }
    ...
    {
        if (empty($this->has_storage_table)) {
            return ['success' => false, 'message' => 'Storage table tidak tersedia.'];
        }
        ...
        if ($this->storage_exists($location_id, $type_id)) {
            ...
            if ($result) {
                if (isset($this->History_model) && method_exists($this->History_model, 'insert_history')) {
                    $historyRes = $this->History_model->insert_history([
                        'electric_id' => $type_id,
                        'type' => 'Masuk',
                        'qty' => $quantity,
                        'user_nik' => $editor_nik,
                    ]);
                    if (empty($historyRes['success'])) {
                        $this->db->trans_rollback();
                        return ['success' => false, 'message' => $historyRes['message'] ?? 'Gagal menyimpan history'];
                    }
                }
                ...
                return ['success' => true, 'message' => 'Barang disimpan'];
            }
            $this->db->trans_rollback();
            return ['success' => false, 'message' => 'Gagal memperbarui storage'];
        } else {
            $insertData = [
            ...
            if ($this->db->field_exists('id', 'as_storage') && isset($this->Id_generator_model)) {
                $insertData['id'] = (int)$this->Id_generator_model->generate_manual_id('as_storage', 'id');
            }
            $insertResult = $this->db->insert('as_storage', $insertData);
            if ($insertResult) {
                if (isset($this->History_model) && method_exists($this->History_model, 'insert_history')) {
                    $historyRes = $this->History_model->insert_history([
                        'electric_id' => $type_id,
            ...
            $this->db->trans_rollback();
            return ['success' => false, 'message' => 'Gagal menyimpan data ke storage'];
        }
    }

    public function take_items($location_id, $type_id, $quantity, $editor_nik)
    {
        if (empty($this->has_storage_table)) return ['success' => false, 'message' => 'Storage table tidak tersedia.'];

        $remaining = (int)$quantity;

        // Build base query for candidate storage rows (prefer specified location if provided)
        $this->db->from('as_storage s');
        $this->db->where('TRIM(s.type_id)', trim($type_id));
        ...
        // Prefer ordering by created_at if available for FIFO, otherwise by id, otherwise by location
        if ($this->db->field_exists('created_at', 'as_storage')) {
            $this->db->order_by('s.created_at', 'ASC');
        } elseif ($this->db->field_exists('id', 'as_storage')) {
            $this->db->order_by('s.id', 'ASC');
        } else {
            $this->db->order_by('s.location_id', 'ASC');
        }

        $candidates = $this->db->get()->result_array();

        if (empty($candidates)) {
            // If we had filtered by location but found nothing, try across all locations
            if (!empty($location_id)) {
                $this->db->from('as_storage s');
                $this->db->where('TRIM(s.type_id)', trim($type_id));
                $this->db->where('s.amount >', 0);
        ...
        foreach ($candidates as $row) {
            ...
            $new_amount = $available - $take;
            $update = $this->db->where('location_id', $row['location_id'])->where('TRIM(type_id)', trim($type_id))->update('as_storage', [
                'amount' => $new_amount,
                'editor' => $editor_nik,
                'updated_at' => date('Y-m-d H:i:s')
            ]);

            // History logging is handled by the caller (History_model) to avoid recursion.
        ...
        if ($remaining > 0) {
            $this->db->trans_rollback();
            return ['success' => false, 'message' => 'Stok tidak mencukupi. Kekurangan: ' . $remaining];
        }
    ...
    {
        if (empty($this->has_storage_table)) return 0;
