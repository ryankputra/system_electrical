...
<body>
    ...
    <div class="kop-surat">
        ...
        <h2>Laporan Hasil Audit Stock Opname</h2>
        <h2>Workshop Automation - Apparel One Indonesia</h2>
        <p>Tanggal Cetak: <?= date('d F Y H:i') ?></p>
        <p>Lokasi Audit: <strong><?= htmlspecialchars($lokasi_nama) ?></strong></p>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="30%">Nama & Spesifikasi Barang</th>
                <th width="15%">Brand</th>
                <th width="15%">Selisih (Discrepancy)</th>
            ...
            <?php if(!empty($rows)): ?>
                <?php $no=1; foreach($rows as $r): ?>
                    <?php
                        $tipe = $r['tipe'] ?? '';
                        $brand = $r['brand'] ?? '';
                        $name = $tipe ? ($tipe . ($brand ? ' - ' . $brand : '')) : ($r['nama'] ?? '');
                    ?>
                    <tr>
                        ...
                        <td class="text-center font-monospace fw-bold"><?= $r['selisih'] ?></td>
                        <td><small><?= htmlspecialchars($r['keterangan']) ?></small></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr>
                    <td colspan="6" class="text-center">Tidak ada penyesuaian audit hari ini.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
