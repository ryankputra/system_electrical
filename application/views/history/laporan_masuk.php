						...
						<?php if(!empty($history)): ?>
							<?php $i=1; $total_semua = 0; foreach($history as $r): ?>
								...
								<tr>
									...
									<td><?= htmlspecialchars($r['po_number'] ?? $r['distributor'] ?? $r['supplier_name'] ?? '-') ?></td>
							...
							<tr class="table-light fw-bold">
								<td colspan="8" class="text-end pe-3">TOTAL KESELURUHAN:</td>
								<td class="text-end">Rp <?= number_format($total_semua, 0, ',', '.') ?></td>
								<td></td>
							</tr>
						<?php else: ?>
							<tr><td colspan="10" class="text-center text-muted p-4">Tidak ada data barang masuk untuk periode ini.</td></tr>
						<?php endif; ?>
					</tbody>
				</table>
			</div>
