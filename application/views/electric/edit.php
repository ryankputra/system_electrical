        ...
        <form action="<?= site_url('electric/edit/' . urlencode($electric['electric_id'])); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <div class="form-control bg-light text-muted">
                    ...
                    // Fetch type name
                    $ci =& get_instance();
                    $typeRow = $ci->db->get_where('as_electric_types', ['id' => $electric['type_id']])->row_array();
                    echo htmlspecialchars($typeRow['type'] ?? 'Unknown', ENT_QUOTES, 'UTF-8');
                    ?>
                </div>
            ...
            <div class="mb-2">
                ...
                <input id="brand" type="text" class="form-control rounded-pill <?= form_error('brand') ? 'is-invalid' : '' ?>" name="brand" placeholder="Contoh: Schneider, Omron" value="<?= set_value('brand', $electric['brand'] ?? ''); ?>" style="text-transform: uppercase;" oninput="this.value = this.value.toUpperCase()">
                <?= form_error('brand', "<div class='invalid-feedback'>", "</div>"); ?>
            </div>

            <div class="mb-2">
                <label for="type" class="form-label">Type / Model</label>
                <input id="type" type="text" class="form-control rounded-pill <?= form_error('type') ? 'is-invalid' : '' ?>" name="type" placeholder="Contoh: EZ9F34106, G2R-1-SND" value="<?= set_value('type', $electric['type'] ?? '') ?>" style="text-transform: uppercase;" oninput="this.value = this.value.toUpperCase()">
                ...
                <div class="col-md-6">
                    <div class="mb-2">
                        ...
                        <div class="input-group">
                            <input id="voltage" type="text" class="form-control <?= form_error('voltage') ? 'is-invalid' : '' ?>" name="voltage" placeholder="Contoh: 220 atau 4-30" value="<?= set_value('voltage', $electric['voltage'] ?? ''); ?>">
                            <select id="voltage_unit" name="voltage_unit" class="form-select" style="max-width: 100px;">
                                <option value="V" <?= set_select('voltage_unit', 'V', ($electric['voltage_unit'] ?? '') === 'V'); ?>>V</option>
                                <option value="VAC" <?= set_select('voltage_unit', 'VAC', ($electric['voltage_unit'] ?? '') === 'VAC'); ?>>VAC</option>
                                <option value="VDC" <?= set_select('voltage_unit', 'VDC', ($electric['voltage_unit'] ?? '') === 'VDC'); ?>>VDC</option>
                            </select>
                        </div>
                ...
                <div class="col-md-6">
                    <div class="mb-2">
                        <label for="ampere" class="form-label">Ampere <small class="text-muted">(opsional)</small></label>
                         <div class="input-group">
                            <input id="ampere" type="number" step="any" class="form-control <?= form_error('ampere') ? 'is-invalid' : '' ?>" name="ampere" placeholder="1.5" value="<?= set_value('ampere', $electric['ampere'] ?? ''); ?>">
            ...
            <div class="mb-2">
                ...
                <div class="input-group">
                    <input id="daya" type="number" step="any" class="form-control <?= form_error('daya') ? 'is-invalid' : '' ?>" name="daya" placeholder="330" value="<?= set_value('daya', $electric['daya'] ?? ''); ?>">
                    <select id="daya_unit" name="daya_unit" class="form-select" style="max-width: 100px;">
                        ...
                        <option value="VA" <?= set_select('daya_unit', 'VA', ($electric['daya_unit'] ?? '') === 'VA'); ?>>VA</option>
            ...
            <div class="mb-3">
                ...
                <select name="location" id="location" class="form-select" required>
                    ...
                    <?php foreach(($locations ?? []) as $loc) : ?>
                        <option value="<?= (int)($loc['id'] ?? 0); ?>" <?= set_select('location', $loc['id'], (int)($electric['location'] ?? 0) === (int)($loc['id'] ?? 0)); ?>>
            ...
            <div class="d-flex justify-content-between mt-4">
                ...
                <a href="<?= site_url('electric/delete/') . urlencode($electric['electric_id'] ?? ''); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus electric ini?')" class="btn btn-danger rounded-pill">Hapus Barang</a>
            </div>
        </form>
    </div>
</div>
