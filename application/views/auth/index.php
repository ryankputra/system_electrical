    ...
    <main style="background-image: url('<?= base_url('assets/img/pabrik.jpg'); ?>'); background-size: cover; background-color: black; padding-top: 9rem;" class="min-vh-100">
        <div class="card rounded-5 shadow border-0 mx-auto w-75" style="max-width: 500px;">
            <div class="card-body d-flex flex-column align-items-center gap-4 p-4">
                ...
                <form action="<?= base_url('auth'); ?>" method="post" class="w-75">
                    
                    <div class="mb-3">
                        <label for="nik" class="form-label">NIK</label>
                        <div class="position-relative">
                            <input id="nik" type="text" class="form-control rounded-pill pe-5 <?= form_error('nik') ? 'is-invalid' : '' ?>" name="nik" placeholder="Masukkan NIK" value="<?= set_value('nik'); ?>" onkeyup="toggleClear('nik', 'clear-button-nik')" autocomplete="off">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="position-relative">
                            <input id="password" type="password" class="form-control rounded-pill pe-5 <?= form_error('password') ? 'is-invalid' : '' ?>" name="password" placeholder="Masukkan Password" autocomplete="off">
...
<script>
    ...
    // Tambahkan notifikasi auto-close jika ada error
    const notification = document.getElementById('notification');
    if (notification) {
        setTimeout(() => {
            const alert = new bootstrap.Alert(notification);
            alert.close();
        }, window.notificationDuration);
    }
</script>
