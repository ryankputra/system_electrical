...
try:
    ...
    # Append the new 4.4.2 and 4.5 (with cleaned math formulas)
    new_442_and_45 = """
...
<p style="text-indent: 0.5in;">Secara teknis, proses rotasi stok dijamin melalui kueri terurut dengan klausa <code>ORDER BY created_at ASC</code>. Kueri ini memprioritaskan baris data pada tabel <code>as_history</code> yang memiliki stempel waktu (<i>timestamp</i>) paling lama untuk dimuat ke dalam memori aplikasi sebagai kepala antrean (<i>front of queue</i>). Ketika transaksi pengeluaran (<i>Outbound</i>) dideklarasikan melalui dokumen <i>Work Order</i> dengan kuantitas tertentu (<i>Q<sub>req</sub></i>), sistem menjalankan perulangan terstruktur (<i>subtraction loop</i>) pada kumpulan baris data terpilih.</p>
<p style="text-indent: 0.5in;">Prosedur transisi status baris data (<i>state machine transition</i>) selama siklus pengurangan stok dirumuskan sebagai berikut: apabila sisa kuantitas pada <i>batch</i> tertua (<i>Q<sub>batch</sub></i>) lebih besar dari sisa permintaan (<i>Q<sub>req</sub></i>), maka sistem akan mengurangi kuantitas <i>batch</i> tersebut (<i>Q<sub>batch</sub></i> &larr; <i>Q<sub>batch</sub></i> - <i>Q<sub>req</sub></i>) dan mempertahankan nilai statusnya sebagai <code>TERSISA</code>. Namun, jika kuantitas permintaan melampaui sisa stok di <i>batch</i> tersebut (<i>Q<sub>req</sub></i> &ge; <i>Q<sub>batch</sub></i>), sistem akan menguras habis stok <i>batch</i> tersebut hingga mencapai titik nol (<i>Q<sub>batch</sub></i> &larr; 0), memicu perubahan status <i>batch</i> secara instan menjadi <code>HABIS</code>, lalu memindahkan sisa permintaan ke antrean <i>batch</i> berikutnya (<i>Q<sub>req</sub></i> &larr; <i>Q<sub>req</sub></i> - <i>Q<sub>batch_old</sub></i>).</p>
...
<p style="text-indent: 0.5in;">Sebagai evaluasi akhir, hasil penelitian ini secara konkret memberikan kontribusi ilmiah untuk jawaban empat pertanyaan penelitian yang dirumuskan pada Bab II:</p>
...
<body>
    <div class="page">
        <div class="note">
            /// DOKUMEN BAB IV LENGKAP DENGAN FORMULA RAPI /// <br>
            File ini berisi naskah BAB IV lengkap yang semua variabel matematika (Q_req, Q_batch) dan simbol logika FIFO-nya sudah dirapikan agar tampak cantik di browser dan dapat langsung disalin ke MS Word.
    ...
    with open(r'C:\Users\Ryan\Documents\TA\diagrams\Revisi_Lengkap_BAB_IV.html', 'w', encoding='utf-8') as f:
        f.write(html_output)
    print("Unified BAB IV successfully generated with cleaned math formulas.")
